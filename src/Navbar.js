...
export default function Navbar() {
    ...
    const filteredCocktails=search.length === 0 ? finalList :finalList.filter(cocktail=>{
    ...
    function alphabeticalList(search, list){
        if(!search){
            ...
            var startsWithChar = (element)=> element.strDrink.toUpperCase().charAt(0)===searchChar;
            var startsWithChar2=startsWithChar
            if(searchChar !== 'Z'){
                startsWithChar2 = (element)=> element.strDrink.toUpperCase().charAt(0)===String.fromCharCode(search.toUpperCase().charCodeAt(0)+1);
   ...
   const formSubmit=(event)=>{
        ...
        setFocused(false);
        setOpened(false);
        navigate('/all',{state:{filteredCocktails: alphabeticalList(search,sortList(filteredCocktails)), search:search}})
   }
      ...
      <nav className="navbar-menu">
        ...
        <div>
          <div className="search-group">
            ...
            > */}
              <SearchIcon className="dropdown-links" onClick={() => setInput(!showInput)}
/>
            {/* </NavLink> */}
          ...
          {(focused || dropdownOpened) && (
            <div ref={ref} className="dropdown">
              <ul>
                {alphabeticalList(search, filteredCocktails)
                  .slice(0, 5)
                  .map((cocktail, i) => {
                    return (
