...
describe.skip('async axios request works',()=>{
    ...
    test('renders cocktail list when API call succeeds', async () => {
        ...
        jest.spyOn(axios, 'all').mockResolvedValue(mockData);

        // axios.all.mockResolvedValue(mockData);
        axios.spread.mockReturnValue(jest.fn());

        render(<Liquor alcohol="Tequila"/>)
        // expect(axios.get).toHaveBeenCalledTimes(1);
        // expect(axios.all).toHaveBeenCalledWith(`https://www.thecocktaildb.com/api/json/v1/1/filter.php?i=/Tequila`);
